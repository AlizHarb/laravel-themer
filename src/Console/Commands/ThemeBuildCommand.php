...
/**
 * Console command to run npm build for a specific theme.
 */
final class ThemeBuildCommand extends Command
{
    use HasThemeOption;
     ...
     * @var string
     */
    protected $description = 'Run npm build for a specific theme workspace';
    ...
    public function handle(ThemeManager $manager): int
    {
        ...
        $this->components->info("Running: npm run build -w @themes/{$theme->slug}");

        $process = new Process(['npm', 'run', 'build', '-w', '@themes/'.$theme->slug], base_path(), null, null, null);
